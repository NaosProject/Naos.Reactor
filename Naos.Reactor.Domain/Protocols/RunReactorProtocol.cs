...
namespace Naos.Reactor.Domain
{
    ...
    public partial class RunReactorProtocol : SyncSpecificVoidProtocolBase<RunReactorOp>
    {
            ...
            RunReactorOp operation)
        {
            ...
            var idsToProcess = new List<StringSerializedIdentifier>();
            foreach (var idToCheck in distinctIds)
            {
                var found = ReactionRegistrationIdToNextEvaluationCutoffMap.TryGetValue(idToCheck, out var nextEvaluationCutoff);

                if (!found || DateTime.UtcNow > nextEvaluationCutoff)
                {
                    idsToProcess.Add(idToCheck);
                }
            }

            var parallelLoopResult = Parallel.ForEach(idsToProcess,
            ...
            if (processedIds.Count != idsToProcess.Count)
            {
                var missedIds = idsToProcess.Where(_ => !processedIds.ContainsKey(_)).ToList();
